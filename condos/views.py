...
from .serializer import (
    ModelCondoSerializer,
    ModelAddressSerializer,
    ModelEmailSerializer,
        ...
        try:
            with transaction.atomic():
                ...
                if condo_data.get("phones"):
                    phone_serializer = ModelPhoneSerializer(data=condo_data["phones"], context={"condo": condo}, many=True)
                    phone_serializer.is_valid(raise_exception=True)
                    phone_serializer.save()

                if condo_data.get("emails"):
                    email_serializer = ModelEmailSerializer(data=condo_data["emails"], context={"condo": condo}, many=True)
